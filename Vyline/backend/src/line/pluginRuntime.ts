...
export function resolvePluginEntry(
  ...
  const dir = isAbsolute(pluginDirName) ? pluginDirName : join(root, pluginDirName);
  const candidates = manifestMain
    ? [join(dir, manifestMain)]
    : [join(dir, "index.ts"), join(dir, "index.js")];
  for (const c of candidates) {
    const rel = relative(dir, c);
    if (!rel.startsWith("..") && !isAbsolute(rel) && existsSync(c)) return c;
  }
  return null;
}
...
export async function activatePlugin(
  ...
  try {
    if (!plugin) {
      ...
      const entry = resolvePluginEntry(sourceDir ?? pluginDirName, manifestMain);
...
export async function deactivatePlugin(accountId: string, pluginId: string): Promise<void> {
  ...
  active.delete(k);
  try {
    await Promise.resolve(entry.plugin.deactivate(entry.context));
    (await makeLogger(pluginId)).info("deactivated");
  } catch {
    /* noop */
  }
}
