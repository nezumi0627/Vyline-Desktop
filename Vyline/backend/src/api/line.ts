...
import {
  ...
  listPlugins,
  setPluginState,
} from "../line/pluginManager.js";
import type { PluginEntry } from "../line/pluginManager.js";
import { activePluginIdsFor } from "../line/pluginRuntime.js";
import {
  createNote,
  deleteNote,
  getNote,
...
import { searchLocalMessages } from "../service/localSearchService.js";
import { getAccountStatus } from "../service/accountStatusService.js";
import {
...
function publicPlugin(plugin: PluginEntry) {
  return {
    id: plugin.id,
    name: plugin.name,
...
lineRouter.get("/:accountId/plugins", async (c) => {
  ...
  return c.json({
    plugins: listPlugins().map((p) => ({
      ...
      active: activePluginIdsFor(accountId).includes(p.id),
    })),
  });
});

lineRouter.post("/:accountId/plugins/install", async (c) => {
  try {
    const body = await c.req.parseBody();
    const file = body.file;
    if (!(file instanceof File)) return c.json({ ok: false, error: "ZIP ファイルが必要です" }, 400);
    const plugin = await installPluginArchive(file);
    return c.json({ ok: true, plugin: publicPlugin(plugin) });
  } catch (err) {
...
lineRouter.post("/:accountId/plugins/:pluginId/:action", async (c) => {
  ...
  try {
    ...
    return c.json({
      ...
      active: activePluginIdsFor(accountId).includes(pluginId),
    });
  } catch (err) {
...
// ─── GET /line/:accountId/search ──────────────

lineRouter.get("/:accountId/search", async (c) => {
  const accountId = c.req.param("accountId");
  const chatMid = c.req.query("chatMid") || undefined;
  const query = c.req.query("query") ?? "";
  const limitRaw = c.req.query("limit") ?? "50";
  const limit = Number(limitRaw);
  if (!Number.isInteger(limit) || limit < 1 || limit > 100) {
    return c.json({ ok: false, error: "limit must be an integer between 1 and 100" }, 400);
  }
  try {
    const results = await searchLocalMessages(accountId, {
      ...(chatMid ? { chatMid } : {}),
      query,
      limit,
    });
    return c.json({ ok: true, query: query.trim(), results, localOnly: true });
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    if (message.startsWith("chatMid") || message.startsWith("query")) {
...
lineRouter.get("/:accountId/status", async (c) => {
  try {
    return c.json(await getAccountStatus(c.req.param("accountId")));
  } catch (err) {
    return handleError(err, c);
  }
});
