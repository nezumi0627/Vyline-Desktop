...
describe("diagnostic redaction", () => {
  test("removes credentials and PII by field name", () => {
    const result = redactForDiagnostics({ authToken: "secret", mid: "u123", text: "hello", count: 2 });
    expect(result).toEqual({ authToken: "[REDACTED_SECRET]", mid: "[REDACTED_PII]", text: "[REDACTED_PII]", count: 2 });
  });
