...
import { writeJsonAtomic, safePathComponent } from "./../storage/safeFile.js";
...
function settingsPath(mid: string): string {
...
export async function saveAccountSettings(
  ...
  await mkdir(join(DATA_DIR, "accounts", safePathComponent(mid)), { recursive: true });
  await writeJsonAtomic(settingsPath(mid), next);
  return next;
}
...
export async function updateSetup(
  ...
  return saveAccountSettings(mid, {
    ...patch,
    setup: { ...current.setup, step: Math.max(0, Math.min(step, totalSteps)), completed, ...(completed ? { completedAt: new Date().toISOString() } : {}) },
  });
}
