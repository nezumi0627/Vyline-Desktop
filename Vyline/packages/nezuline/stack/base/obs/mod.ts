...
export class LineObs {
	...
	public async uploadMediaByE2EE(options: {
		...
		// Try E2EE path first; fall back to plain if peer doesn't support E2EE
		...
		try {
			...
			if (e instanceof Error && (e.name === "Not support E2EE" || e.message?.startsWith("Not support E2EE"))) {
				e2ee = false;
				chunks = [];
			} else {
				throw e;
