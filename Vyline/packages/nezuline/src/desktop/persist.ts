...
import fallbackProfile from "../../data/desktop-profile.fallback.json";

const _here = dirname(fileURLToPath(import.meta.url));
const FALLBACK_PATH = join(_here, "../../data/desktop-profile.fallback.json");

export function defaultNezuDataDir(override?: string): string {
  if (override) return override;
  if (process.env["VYLINE_DATA_DIR"]) {
    return join(process.env["VYLINE_DATA_DIR"], "nezuline");
  }
  return join(_here, "../../.cache");
}
...
export function loadFallbackProfile(): DesktopProfile {
  // 1. 配布時: 同梱先を差し替えたい場合
  const envPath = process.env["VYLINE_FALLBACK_PROFILE"];
  if (envPath && existsSync(envPath)) {
    try {
      return JSON.parse(readFileSync(envPath, "utf8")) as DesktopProfile;
    } catch {
      /* ignore */
    }
  }
  // 2. dev: リポジトリ内の fallback ファイル
  ...
  // 3. コンパイル exe: 同梱デフォルト（bun が JSON を inline する）
  return fallbackProfile as unknown as DesktopProfile;
}
