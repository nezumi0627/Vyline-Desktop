...
> ✅ **実装状況**: 抽出パッケージ、バックエンド、ウィザード UI、API クライアントが実装済みです。
> Vyline GUI から、暗号化された iPhone ローカルバックアップの復元を利用できます。
> 設定 > 詳細・復元から、暗号化された iPhone ローカルバックアップを復元できます。

> ⚠️ **重要**: **必ずプライマリデバイス（メインの iPhone）で操作してください。**  
> サブデバイス（iPad、セカンダリ iPhone、LINE Desktop 等）では **トークのバックアップ作成ができません**。

> ✅ **メディア（画像・動画・音声・ファイル・スタンプ）の復元に対応しています。**
...
**対応しています**。復元結果には、テキスト・メタデータに加えて復元できたメディア件数が表示されます。
...
- **メディア**: 対応済み。復元結果の `media.restored` に復元件数が記録されます
