...
async function rightClick(page: Page, runtime: Runtime, locator: Locator): Promise<void> {
  ...
  const menu = page.getByRole("menu", { name: "操作メニュー" });
  try {
    await menu.waitFor({ state: "visible", timeout: 1_500 });
  } catch {
...
async function menuItem(page: Page, runtime: Runtime, name: string): Promise<void> {
  await click(page, runtime, page.getByRole("menuitem", { name, exact: false }).first());
}
...
const scenarios: Scenario[] = [
  ...
  },
  {
    ...
    run: async (page, runtime) => {
      ...
      await click(page, runtime, page.getByTitle("✨"));
    },
  },
  {
  ...
  },
  {
    ...
    run: async (page, runtime) => {
      ...
      await click(page, runtime, page.getByRole("button", { name: /ミュートメッセージ: 無効/ }));
      await click(page, runtime, page.getByRole("button", { name: /ミュート送信/ }));
    },
  },
  {
    id: "in-chat-search",
  ...
  },
  {
    ...
    run: async (page, runtime) => {
      ...
      await page.getByRole("heading", { name: "Vyline開発チーム" }).waitFor();
  ...
  },
  {
    ...
    run: async (page, runtime) => {
      ...
      await click(page, runtime, page.getByRole("button", { name: "ミュート", exact: true }));
  ...
  },
  {
    ...
    run: async (page, runtime) => {
      ...
      await pause(700);
      await click(page, runtime, page.getByRole("button", { name: "まとめて送信", exact: true }));
      await page
        .locator(".vy-chat-messages")
  ...
  },
  {
    ...
    run: async (page, runtime) => {
      await click(page, runtime, page.getByRole("tab", { name: "グループ" }));
      await click(page, runtime, page.getByRole("button", { name: "グループを作成" }).first());
      ...
      await click(page, runtime, page.getByRole("button", { name: "グループを作成" }).last());
      await page.getByRole("button", { name: /新製品デモチーム 2人/ }).waitFor();
    },
  },
  {
    id: "settings-profile",
    ...
    run: async (page, runtime) => {
      ...
      await click(page, runtime, page.getByRole("button", { name: "LINE に保存" }));
  ...
  },
  {
    ...
    run: async (page, runtime) => {
      ...
      await click(page, runtime, page.getByRole("button", { name: /フェザー/ }));
  ...
  },
  {
    ...
    run: async (page, runtime) => {
      ...
      page.once("dialog", (dialog) => dialog.accept());
      await click(page, runtime, page.getByRole("button", { name: "削除" }).first());
    },
  },
  {
    id: "settings-info",
...
async function openDemo(): Promise<{
  ...
  await page.route("**/*", async (route) => {
    ...
    } else if ((localVite && !url.pathname.startsWith("/api/")) || safeInline) {
  ...
  // 前のChrome録画窓が閉じた直後でもOpenScreenの列挙へ確実に載せる。
  await pause(1_800);
...
async function decorateProject(
  ...
  project.editor ??= {};
  project.editor.cropRegion = { x: 0, y: 0.075, width: 1, height: 0.925 };
  project.editor.zoomRegions ??= [];
  project.editor.annotationRegions ??= [];
  // 冒頭タイトルを読めるようにしてから、操作箇所へ緩やかに寄る。
  let previousEnd = 2_200;
  runtime.focusPoints.slice(0, 3).forEach((point, index) => {
    const startMs = Math.max(previousEnd + 180, point.atMs - 420);
    const endMs = Math.min(scenario.duration * 1000 - 350, startMs + 950);
    ...
    project.editor!.zoomRegions!.push({
      ...
      depth: 1.32,
      focus: {
        cx: Math.max(0.16, Math.min(0.84, point.x)),
        cy: Math.max(0.14, Math.min(0.86, point.y)),
      },
      focusMode: "manual",
      source: "manual",
    });
  ...
  project.editor.annotationRegions.push({
    id: "vyline-title",
    startMs: 250,
    endMs: Math.min(2_000, scenario.duration * 1000 - 300),
    type: "text",
...
async function finishHighQuality(
  rawPath: string,
  finalPath: string,
...
async function main(): Promise<void> {
  ...
  try {
    for (const [selectedIndex, scenario] of selected.entries()) {
      ...
      let clipDurationSeconds = scenario.duration;
      try {
        if (!dryRun) {
          // UIの読み込みが遅い環境でも途中で録画が切れない上限。操作完了後はstdinで即停止する。
          recorder = record(projectPath, scenario.duration + 12);
          runtime.startedAt = Date.now();
          await pause(900);
        }
        await scenario.run(page, runtime);
        await pause(1_250);
        clipDurationSeconds = Math.max(3, (Date.now() - runtime.startedAt) / 1000);
        ...
        if (recorder && recorder.exitCode == null) {
          try {
            ...
            // duration上限と同時に終了した場合は既にstdinが閉じている。
          }
        }
      ...
      if (!dryRun) {
        ...
        await finishHighQuality(rawPath, finalPath, clipDurationSeconds);
