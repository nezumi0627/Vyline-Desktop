...
export function IosBackupBetaPanel({ accountId }: { accountId: string | null }) {
  ...
  const start = async () => {
    if ((!accountId && !demoMode) || !selected || !password) return;
    setLoading(true);
    setMessage(null);
    setSession({
      id: "",
    ...
    if (demoMode) {
      window.setTimeout(() => {
        setSession({
          id: "demo-restore-session",
          status: "running",
          progress: {
            stage: "messages",
            current: 680,
            total: 1248,
            message: "トーク履歴とメディアを復元しています",
          },
          result: null,
          error: null,
          startedAt: Date.now(),
      ...
      window.setTimeout(() => {
        ...
        setSession({
          ...
          result: {
            ...
            backupDate: new Date(completedAt - 86_400_000).toISOString(),
            restoredAt: new Date(completedAt).toISOString(),
            extracted: { lineFiles: 18, databases: 3 },
            parsed: { chats: 24, totalMessages: 1248 },
            restoredChatMids: [],
            media: { restored: 86, skipped: 0 },
          },
          error: null,
          startedAt: completedAt - 1800,
          completedAt,
        });
        setLoading(false);
        setPassword("");
  ...
  return (
    <section className="mt-6 rounded-2xl border border-amber-500/40 bg-amber-500/5 p-4">
      ...
      <p className="mt-3 text-[0.65rem] text-[var(--vy-text-dim)]">
        iOSバックアップ復元は通常機能です。元のバックアップは変更しません。
      </p>
    </section>
  );
}
