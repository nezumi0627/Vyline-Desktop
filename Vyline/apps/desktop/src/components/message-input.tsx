...
export function MessageInput({ chatId }: { chatId: string }) {
  ...
  async function sendPendingMedia() {
    if (sendingMediaBatch || pendingMedia.length === 0 || (!accountId && !demoMode)) return;
    setSendingMediaBatch(true);
    try {
      if (demoMode) {
        for (const item of pendingMedia) await sendImageFile(chatId, item.file);
        clearPendingMedia();
        return;
      }
