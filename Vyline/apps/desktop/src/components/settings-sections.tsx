...
import { LocalSearchSection } from "@/components/local-search-section";
...
const NAV: { key: Section; label: string; icon: React.ReactNode }[] = [
  ...
  { key: "plugins", label: "プラグイン", icon: <IconCopyCode size={18} /> },
  { key: "search", label: "検索", icon: <IconSearch size={18} /> },
  { key: "account", label: "アカウント状態", icon: <IconHardDrive size={18} /> },
];
        ...
        <div className="flex w-full flex-col overflow-hidden">
          ...
          <div className="vy-scroll flex-1 overflow-y-auto px-4 py-6 md:px-8">
            <div key={section} className="vy-section-enter mx-auto max-w-2xl">
              ...
              {section === "plugins" && (
                <PluginsSection accountId={accountId} demoMode={demoMode} />
              )}

              {section === "search" && <LocalSearchSection />}

              {section === "account" && <AccountStatusSection />}
            </div>
          </div>
        </div>
      </div>
...
type PluginItem = Awaited<ReturnType<typeof api.line.plugins>>["plugins"][number];

function PluginsSection({ accountId, demoMode }: { accountId: string | null; demoMode: boolean }) {
  ...
  const toggle = async (plugin: PluginItem) => {
    if (!accountId || demoMode || !plugin.loadable) return;
    setBusyId(plugin.id);
    setMessage(null);
    try {
      const result = await api.line.setPlugin(accountId, plugin.id, !plugin.enabled);
      if (!result.ok) throw new Error(result.error ?? "プラグインの切替に失敗しました");
      setPlugins((current) =>
        current.map((item) =>
          item.id === plugin.id
            ? { ...item, enabled: result.enabled, active: result.active }
            : item,
        ),
      );
      setMessage(`${plugin.name} を${result.enabled ? "有効化" : "無効化"}しました`);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : String(error));
    } finally {
      setBusyId(null);
  ...
  const install = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setMessage(null);
    try {
      const result = await api.line.installPlugin(accountId, file);
      if (!result.ok) throw new Error(result.error ?? "プラグインのインストールに失敗しました");
      await load();
      setSelectedId(result.plugin?.id ?? null);
      setMessage(`${result.plugin?.name ?? "プラグイン"} をインストールしました`);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : String(error));
    } finally {
      setInstalling(false);
  ...
  return (
    <Section title="プラグイン" desc="Vyline の動作を ES Module で拡張します">
      <div className="mb-4 rounded-2xl border border-[var(--vy-border)] bg-[var(--vy-surface-2)] p-4 text-sm">
        <div className="flex items-center justify-between gap-3">
          <p className="font-medium">プラグインを追加</p>
          <label
            className={cn(
              "cursor-pointer rounded-lg bg-[var(--vy-accent)] px-3 py-2 text-xs font-semibold text-[var(--vy-accent-contrast)]",
              "hover:opacity-90",
              (installing || !accountId || demoMode) && "pointer-events-none opacity-50",
            )}
          >
            {installing ? "検証中…" : "ZIP を選択"}
            <input
              type="file"
              accept=".zip,application/zip"
              className="hidden"
              onChange={(event) => void install(event)}
              disabled={installing || !accountId || demoMode}
            />
          </label>
        </div>
        <p className="mt-1 text-xs leading-relaxed text-[var(--vy-text-dim)]">
          ZIP 内に manifest.json と index.ts（または index.js）を含めてください。安全性を検証して
          <code>backend/data/plugins/</code> にインストールします。既存 ID は上書きしません。
        </p>
      </div>
      {loading ? (
        <div
          role="status"
          className="rounded-2xl border border-[var(--vy-border)] p-5 text-sm text-[var(--vy-text-dim)]"
        >
          プラグインを読み込み中…
        </div>
      ) : plugins.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-[var(--vy-border)] p-6 text-center">
          <p className="text-sm font-medium">利用可能なプラグインはありません</p>
          <p className="mt-1 text-xs text-[var(--vy-text-dim)]">
            backend/data/plugins に追加するとここに表示されます。
          </p>
        </div>
      ) : (
        <Card>
            ...
            <div
              ...
              onKeyDown={(event) => {
                if (event.key === "Enter" || event.key === " ")
                  setSelectedId(selectedId === plugin.id ? null : plugin.id);
              }}
              className="cursor-pointer rounded-xl px-3 transition-colors hover:bg-[var(--vy-surface-2)]"
            >
              <Row
                title={plugin.name}
                desc={`${plugin.description ?? "説明なし"} · v${plugin.version}`}
              >
                <div
                ...
                >
                  <span className="text-[0.65rem] text-[var(--vy-text-dim)]">
                    {plugin.active ? "実行中" : plugin.loadable ? "停止中" : "読込不可"}
                  </span>
                  <Toggle
                    checked={plugin.enabled}
                    onChange={() => void toggle(plugin)}
                    disabled={busyId === plugin.id || !plugin.loadable || demoMode}
                    label={`${plugin.name}を${plugin.enabled ? "無効化" : "有効化"}`}
                  />
                </div>
              </Row>
              {selectedId === plugin.id && (
                <div className="mb-3 rounded-xl bg-[var(--vy-surface-2)] p-3 text-xs text-[var(--vy-text-dim)]">
                  <p className="font-medium text-[var(--vy-text)]">詳細</p>
                  <dl className="mt-2 grid grid-cols-[auto_1fr] gap-x-3 gap-y-1">
                    <dt>バージョン</dt>
                    <dd>{plugin.version}</dd>
                    <dt>権限</dt>
                    <dd>{plugin.permissions?.length ? plugin.permissions.join(", ") : "なし"}</dd>
                    <dt>読込可能</dt>
                    <dd>{plugin.loadable ? "はい" : "いいえ"}</dd>
                    <dt>状態</dt>
                    <dd>
                      {plugin.active ? "active" : plugin.enabled ? "enabled（停止中）" : "disabled"}
                    </dd>
                    <dt>インストール元</dt>
                    <dd>{plugin.source === "bundled" ? "同梱" : "ZIP / backend data"}</dd>
                  </dl>
                  <p className="mt-2 leading-relaxed">{plugin.description ?? "説明なし"}</p>
                </div>
              )}
            </div>
          ))}
      ...
      {message && (
        <p role="status" className="mt-3 text-xs text-[var(--vy-text-dim)]">
          {message}
        </p>
      )}
    </Section>
