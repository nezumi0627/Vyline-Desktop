...
export const api = {
  ...
  line: {
    ...
    installPlugin: (accountId: string, file: File) => {
      const form = new FormData();
      form.append("file", file);
      return request<{ ok: boolean; plugin?: PluginInstallResult; error?: string }>(
        "POST",
        `/line/${encodeURIComponent(accountId)}/plugins/install`,
        form,
      );
    },

    setPlugin: (accountId: string, pluginId: string, enabled: boolean) =>
      request<{ ok: boolean; pluginId: string; enabled: boolean; active: boolean; error?: string }>(
        "POST",
    ...
    searchMessages: (
      accountId: string,
      query: string,
      opts?: { chatMid?: string; limit?: number },
    ) => {
      const q = new URLSearchParams({ query, limit: String(opts?.limit ?? 50) });
      if (opts?.chatMid) q.set("chatMid", opts.chatMid);
      return request<{
        ok: boolean;
        query?: string;
        localOnly?: boolean;
  ...
  settings: {
    account: (mid: string) =>
      request<{ ok: boolean; settings: AccountSettings }>("GET", `/settings/accounts/${encodeURIComponent(mid)}`),
    saveAccount: (mid: string, settings: Partial<AccountSettings>) =>
      request<{ ok: boolean; settings: AccountSettings }>("PUT", `/settings/accounts/${encodeURIComponent(mid)}`, settings),
    saveSetup: (mid: string, step: number, settings: Partial<AccountSettings>) =>
      request<{ ok: boolean; settings: AccountSettings }>("PATCH", `/settings/accounts/${encodeURIComponent(mid)}/setup`, { step, settings }),
  },
};
