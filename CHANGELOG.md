...
- **プラグイン** — 設定画面から ZIP を選択して安全にインストール。manifest、権限、実行状態の詳細確認と有効化/無効化に対応
- **トーク横断検索** — ローカル履歴を対象に、チャット名・送信者・時刻・抜粋付きで検索
- **アカウント状態** — アカウント別の保存容量、ログイン状態、保存セッション状態を表示
- **iOS バックアップ復元** — ベータ機能から詳細・復元へ移動し、テキスト・メタデータ・メディア復元の案内を更新

### 改善

- LAN_ACCESS 時に Vite を LAN bind し、proxy 経由の接続元判定を修正
- 通話 UI を非表示化。通話プロトコルと API は内部実装として保持
- OpenChat / Square メッセージ API を Coming Soon（実装予定）として明示
- CI に全テスト実行を追加
