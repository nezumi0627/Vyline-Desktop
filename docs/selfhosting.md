...
開発構成（`bun run dev`）で LAN 端末から接続する場合は、リポジトリ直下の `.env` に
`VYLINE_LAN_ACCESS=true` を設定して backend / frontend を再起動し、
`http://<PCのLANアドレス>:5173` を開きます。Vite も同じ `.env` を読むため UI が
`127.0.0.1` に閉じず、`/api` は backend の `:3001` へ proxy されます。LAN 端末の
API はサブデバイスセッションが必要で、pairing の確認・完了だけが未認証で許可されます。
