...
2. UI 細部改善（継続）
3. stack RPC の Desktop 準拠への段階的置換
...
| 機能                 | 内容                                                                    | 方針メモ                                          |
| -------------------- | ----------------------------------------------------------------------- | ------------------------------------------------- |
| **プラグイン API**   | ES Modules で UI・挙動を動的拡張                                        | ZIP インストール・権限表示・実行切替を実装済み。UI拡張 API は継続拡張 |
| **オープンチャット** | LINE オープンチャット（不明な相手と交流するチャットルーム）の閲覧・参加 | **Coming Soon（実装予定）**。現時点で UI 導線なし。 |
| **Square メッセージ API** | Square 系メッセージの取得・送受信 | **Coming Soon（実装予定）**。現時点では未提供。 |
