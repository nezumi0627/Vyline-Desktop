...
| 現在 | **GUI/API 統合済み** | 暗号化ローカルバックアップの検出・復号・履歴／メディア復元を GUI から実行できる |
...
- **メディア復元**: 対応済み。復元結果のメディア件数を GUI に表示する
...
> ✅ **メディア（画像・動画・音声・ファイル・スタンプ）の復元に対応**
...
- [x] `backend/src/api/line.ts` — iOS バックアップ BFF ルート
...
| メディア | 復元処理に対応済み。実機バックアップの対象差分は継続検証 |
...
Vyline/backend/src/tools/iosLineExtract.ts     # 現行実装を使用
Vyline/backend/src/service/iosBackupService.ts # 現行実装を使用
Vyline/backend/src/api/line.ts                 # iOS ルート実装済み
