...
<!--ja-->
<p align="center"><!--ja-->
  <img alt="version" src="https://img.shields.io/badge/version-0.1.0--beta.6-a78bfa?style=flat-square" /><!--ja-->
...
| **通話** | プロトコルのみ保持。Desktop UI は非表示、実装予定 |<!--ja-->
...
| **プラグイン** | ZIP インストール、権限表示、実行切替、ES Module 拡張 |<!--ja-->
| **検索** | ローカル履歴のトーク横断メッセージ検索 |<!--ja-->
| **アカウント状態** | アカウント別データ容量・ログイン/保存セッション状態 |<!--ja-->
| **VylineBackup** | トーク履歴とメディアのスナップショット作成・復元・削除 |<!--ja-->
| **開発者向け** | Bearer トークン対応の公開 API、OpenAPI 3.1、JSONL 詳細ログ |<!--ja-->
| **その他** | Keepメモ、プロフィール背景、共通グループの高速表示、トークの TXT 保存 |<!--ja-->
<!--ja-->
---<!--ja-->
<!--ja-->
## Vyline を支援する<!--ja-->
...
README は `README.src.md` を正本とし、`bun run docs:readme` で日本語版（デフォルト）と英語版（`README.en.md`）を生成します。生成物を直接編集せず、`bun run docs:readme:check` で同期状態を確認してください。<!--ja-->
<!--ja-->
---<!--ja-->
<!--ja-->
## 解析ツールキット<!--ja-->
